...
class TestDjangoHw5simple(unittest.TestCase):
    ...
    def get_csrf_login_token(self, session=None):
        ...
        response0 = session.get("http://localhost:8100/accounts/login/")
        ...
        self.loginheaders = {"X-CSRFToken": csrfdata, "Referer":
                "http://localhost:8100/accounts/login/"}
    ...
    def setUpClass(cls):
        ...
        def login(data):
            response = requests.post("http://localhost:8100/app/createUser",
                                     data=data,
                                     )
            ...
            loginheaders = {"X-CSRFToken": csrfdata, "Referer":
                            "http://localhost:8100/accounts/login/"}
            response1 = session.post("http://localhost:8100/accounts/login/",
                        data=logindata,
...
#           "Content:{}".format(request.text)
    ...
    def test_create_post_user_success(self):
        '''Test createPost endpoint by a user, which should succeed http://localhost:8100/app/createPost'''
        ...
        self.assertNotEqual(request.status_code, 404,
            "Server returned 404 not found for http://localhost:8100/app/createPost " +
            "Data:{}".format(data)
...
#           + "Content:{}".format(request.text)
    ...
    def test_hide_post_notloggedin(self):
        '''Test hidePost endpoint not logged in, which should fail with 401 unauthorized http://localhost:8100/app/hidePost'''
        data = {'post_id': "0",  "reason": "天安门广场" }
        session, csrf = self.get_csrf_login_token()
        request = session.post(
            "http://localhost:8100/app/hidePost",
             data=data)
        self.assertNotEqual(request.status_code, 404,
            "Server returned 404 not found for http://localhost:8100/app/hidePost " +
            "Data:{}".format(data)
#           "Content:{}".format(response2.text)
            )
...
#           + "Content:{}".format(request.text)
    ...
    def test_hide_post_user_unauthorized(self):
        '''Test hidePost endpoint by a user, which should fail with 401 unauthorized http://localhost:8100/app/createPost'''
...
#           + "Content:{}".format(request.text)
    ...
    def test_hide_post_admin_success(self):
        '''Test hidePost endpoint by an admin which should succeed http://localhost:8100/app/hidePost'''
        data = {'post_id': "1",  "reason": "NIXON",
...
#           + "Content:{}".format(request.text)
    ...
    def test_create_comment_admin_success(self):
        ...
        response2 = session.post(
            "http://localhost:8100/app/createComment",
            data=data,headers=self.headers_admin)
#        404 pages are too bulky to show in gradescope
        self.assertNotEqual(response2.status_code, 404,
            "Server returned 404 not found for http://localhost:8100/app/createComment " +
            "Data:{}".format(data)
#           "Content:{}".format(response2.text)
            )
        self.assertEqual(response2.status_code, 200,
            "Server returns error for http://localhost:8100/app/createComment " +
            "Data:{}".format(data)+
            "Content:{}".format(response2.text)
            )
...
#           "Content:{}".format(response2.text)
        ...
        self.assertEqual(response2.status_code, 200,
            "Server returned an error for http://localhost:8100/app/createComment " +
            "Data:{}".format(data) +
            "Content:{}".format(response2.text)
            )
    ...
    def test_login_index(self):
        ...
        response_index = session.get("http://localhost:8100/")
        ...
        self.assertLess(response_index.status_code, 203,
                         "Server returns error for GET to http://localhost:8100/ " +
    ...
    def test_create_post_add(self):
        ...
        print("Calling http://localhost:8100/app/createPost with", data)
        response2 = session.post("http://localhost:8100/app/createPost",
                                 data=data, headers=self.headers_admin)
    ...
    def test_create_comment_add(self):
        ...
        response2 = session.post("http://localhost:8100/app/createComment",
                                 data=data, headers=self.headers_admin)
        after_rows = self.count_app_rows()
        self.assertGreater(after_rows - before_rows, 0,
